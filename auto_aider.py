...
def main():
    ...
    for i, task in enumerate(tasks, 1):
        ...
        try:
            ...
            task_results.append({"task": i, "status": "success", "output": result.stdout})
        except subprocess.CalledProcessError as e:
            print(f"Error executing task {i}: {e}")
            task_results.append({"task": i, "status": "failure", "error": str(e), "output": e.output})

    print("\nTask Execution Summary:")
    for result in task_results:
        print(f"Task {result['task']}: {result['status'].capitalize()}")

    # Optionally, you can save the detailed results to a file
